...
void HojaCalc::insNodo(void)
{
    ...
    do
    {
        ...
        if (fila >= 1 && fila <= totFil && columna >= 1 && columna <= totCol)
        {
            ...
            cout << "Hubicacion no válida para nuevo dato" << endl
                 << endl;
...
void HojaCalc::sumacolumnas(int colum){
     nodo *s = pHE;
   do{
       if(s->nCol == colum){
           int sum =0;
           ...
           cout <<"Suma de la columna "<<colum<<": "<< sum << endl;
           break;
       }
       s =s->sigCol;
   }while(s->nCol != 0);
...
int main(void)
{
    ...
    cout << "cantidad de filas y columnas que contendra:" << endl<< endl;
    cout << "Total de filas: ";
    cin >> totFilas;
